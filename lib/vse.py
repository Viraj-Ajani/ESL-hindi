...
if heuristic_strategy:
    ...
    class Image_levels(nn.Module):
        ...
        def get_image_levels(self, img_emb, batch_size, n_region):
            ...
            sub_space_index = torch.arange(1024, device=device, dtype=torch.long)
...
class sims_claculator(nn.Module):
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        sim_all = []
        batch_size, n_region, embed_size = img_emb.size(0), img_emb.size(1), img_emb.size(2)
        sub_space_index = torch.arange(self.sub_space, device=device, dtype=torch.long)
...
class VSEModel(object):
    ...
    def __init__(self, opt):
        ...
        self.txt_enc = get_text_encoder(opt.embed_size, token=opt.token, no_txtnorm=opt.no_txtnorm)
    ...
    def make_data_parallel(self):
        ...
        # allow disabling DP via env var, e.g., ESL_DISABLE_DATAPARALLEL=1
        dp_disabled = os.getenv("ESL_DISABLE_DATAPARALLEL", "0") == "1"
        use_dp = use_cuda and ngpu > 1 and not dp_disabled

        if use_dp:
            device_ids = list(range(ngpu))
            self.img_enc = nn.DataParallel(self.img_enc, device_ids=device_ids, output_device=device_ids[0])
            self.txt_enc = nn.DataParallel(self.txt_enc, device_ids=device_ids, output_device=device_ids[0])
            self.sim_vec = nn.DataParallel(self.sim_vec, device_ids=device_ids, output_device=device_ids[0])
            print(f"[ESL] Using DataParallel on GPUs: {device_ids}")
        else:
            print(f"[ESL] Not using DataParallel (cuda={use_cuda}, ngpu={ngpu}, disabled={dp_disabled}).")
    ...
    def forward_emb(self, images, captions, lengths, image_lengths=None):
        ...
        if self.opt.precomp_enc_type == 'basic':
            if torch.cuda.is_available():
                ...
                image_lengths = image_lengths.to(device)
