...
def get_text_encoder(embed_size, token, no_txtnorm=False):
    return EncoderText(embed_size, token, no_txtnorm=no_txtnorm)
...
class EncoderText(nn.Module):
    def __init__(self, embed_size, token, no_txtnorm=False):
        super(EncoderText, self).__init__()
        self.embed_size = embed_size
        self.no_txtnorm = no_txtnorm

        self.bert = AutoModel.from_pretrained(token)
    ...
    #     print("bert_emb shape:", bert_emb.shape)
    ...
    def forward(self, x, lengths):
        ...
        # IndicBERT uses the same interface as BERT
        outputs = self.bert(
            input_ids=x,
            attention_mask=bert_attention_mask,
            return_dict=True  # Ensures consistent output format
        )
        
        # Get the last hidden states (sequence output)
        bert_emb = outputs.last_hidden_state
